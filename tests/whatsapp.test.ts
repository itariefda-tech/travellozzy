...
void test('buildBookingMessage emits every required booking field', () => {
  ...
  assert.match(message, /Pilihan kendaraan: Family Comfort/);
...
void test('buildWhatsAppUrl safely encodes inquiry text', () => {
  const url = buildWhatsAppUrl('Halo TRAVELOZZY\nAirport & Wedding');
  assert.ok(url.startsWith('https://wa.me/'));
  assert.match(url, /Halo%20TRAVELOZZY%0AAirport%20%26%20Wedding/);
});
