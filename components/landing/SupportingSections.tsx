...
export function WhyTravelozzy() {
  return (
    <section className="section section--white" id="why" aria-labelledby="why-title">
      <div className="container-shell why-layout">
        <div><p className="eyebrow">Journey with confidence</p><h2 id="why-title">A thoughtful ride, from first message to arrival.</h2><p>TRAVELOZZY membantu menerjemahkan kebutuhan perjalanan menjadi kelas kendaraan dan model layanan yang lebih tepat.</p></div>
...
export function CorporateSection() {
  ...
  return (
    <section className="corporate-section" id="corporate" aria-labelledby="corporate-title"><div className="container-shell corporate-grid"><div><BriefcaseBusiness aria-hidden="true" /><p className="eyebrow">TRAVELOZZY Business</p><h2 id="corporate-title">Transportation that works as professionally as you do.</h2><p>Inquiry untuk executive, guest, operational, event, airport, atau perjalanan lintas kota.</p><a className="button-link button-link--gold" href="#booking">Corporate Inquiry <ArrowRight /></a></div><ul>{items.map((item, index) => <li key={item}><span>0{index + 1}</span>{item}</li>)}</ul></div></section>
  );
}

export function PartnerSection() {
  return (
    <section className="partner-section" aria-labelledby="partner-title"><div className="container-shell partner-inner"><Handshake aria-hidden="true" /><div><p className="eyebrow">TRAVELOZZY Partner</p><h2 id="partner-title">Grow with every journey.</h2><p>Partnership inquiry untuk hotel, wedding organizer, event organizer, travel agent, corporate, dan tour guide.</p></div><a className="text-link" href="#booking">Become a Partner <ArrowRight /></a></div></section>
  );
}
